function __changeUserName(of) {
    var username = $('#' + of).val();
    if (of == 'tel') {
        if (username.search(/^1[3|4|5|8|7][0-9]\d{8}$/) == -1) {
            showTooltips('tel_input', '请输入正确的手机号');
            return;
        }
    } else {
        if (username == '' || !isMobilePhone(username)) {
            showTooltips('tel_input', '请输入正确的邮箱');
            return;
        }
    }
}
...
function __changeUserName1(of) {
    var Field_UserName = $('#' + of).val();
    var chinese = /^[\u4E00-\u9FA5]+$/i;
    if (of == 'Field_UserName') {
        if (Field_UserName.search(chinese) == -1) {
        	if(Field_UserName   == ''){
        		showTooltips('mobile_input', '请输入正确的手机号/用户名');
						return;
        	}
        }else{
        	 showTooltips('mobile_input', '请输入正确的手机号/用户名');
...
function checkPwd(pwd) {
    ...
    if (passwordRegex.test(pwd) == false) {
        showTooltips('password_input', '密码位数太少或格式不正确');
    } else {
        var msg = checkPassword(pwd);
        if (msg == "red") {
            showTooltips('password_input', '您的密码风险极大，请重新设置。');
						return;
        }
        else if (msg == "orange") {
            showTooltips('password_input', '您的密码过短。请使用8-15位数字加字母的组合方式。');
						return;
        }
        else if (msg == "yellow") {
            showTooltips('password_input', '您的密码过于简单，请使用数字加字母的组合方式。');
						return;
        } else {
...
function checkRate(rating) {
    ...
    if (rating == "") {
        showTooltips('rate_input', '请输入你的级别');
    } else {
        hideTooltips('rate_input');
    }
}
