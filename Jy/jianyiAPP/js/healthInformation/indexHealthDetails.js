...
//				console.log("initCommentList---->:"+JSON.stringify(response));
				if (response.code == 1) {
					...
					for (var i = 0; i < dataValues.length; i++) {
						...
						var content = dataValues[i].Content ? dataValues[i].Content : "";
	...
	submitComment: function() {
		...
		if(!commomUtil.validateCommon(commentContent)){
			plus.nativeUI.alert('评论内容不能包含特殊字符','','健医宝','确认');
			return false;
		}
...
//						//开启上拉加载
						...
						plus.nativeUI.alert("评论内容成功", "", "健医宝", "OK");
						nav.style.display="block";
						return;
					}
				},
				onerror: function(e) {
					submitflag = false;
					plus.nativeUI.alert("评论提交失败", "", "健医宝", "确认");
			...
			submitflag = false;
			plus.nativeUI.alert("请填写评论内容", "", "健医宝", "确认");
