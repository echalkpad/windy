...
var pageSize = 10;
var total = 0;
var _commentdivs = document.getElementById('slowExpertsDetailsCommentsID');
var commentDID = "";
//添加newId自定义事件监听
window.addEventListener('mainInterrLoadSlowExpertsList', function(event) {
	//获得事件参数
	doctorId = event.detail.slowExpertsIdMainInterr;
	//根据id向服务器请求新闻详情
	slowExpertsDetails.initSlowExpertsDetails(doctorId);
});
//添加newId自定义事件监听
window.addEventListener('loadSlowExpertsList', function(event) {
	//获得事件参数
	doctorId = event.detail.slowExpertsIdList;
	//根据id向服务器请求新闻详情
	slowExpertsDetails.initSlowExpertsDetails(doctorId);
});
//添加newId自定义事件监听
window.addEventListener('loadSlowExpertsListSearch', function(event) {
	//获得事件参数
	doctorId = event.detail.slowExpertsIdSearch;
	//根据id向服务器请求新闻详情
	slowExpertsDetails.initSlowExpertsDetails(doctorId);
});
//添加newId自定义事件监听
window.addEventListener('loadSlowExpertsListDoctorList', function(event) {
	//获得事件参数
	doctorId = event.detail.slowExpertsIdDoctorList;
	//根据id向服务器请求新闻详情
	slowExpertsDetails.initSlowExpertsDetails(doctorId);
});

var slowExpertsDetails = {
	initSlowExpertsDetails: function(doctorId) {
		//设置显示tab
		document.getElementById('itemA1').setAttribute('class','mui-control-item mui-active');
		document.getElementById('itemA2').setAttribute('class','mui-control-item');
		document.getElementById('itemA3').setAttribute('class','mui-control-item');
		...
		document.getElementById('slowExpertsDetailsSkilledID').innerHTML = '';
		document.getElementById('slowExpertsDetailsCareerID').innerHTML = '';
...
//				console.log("initSlowExpertsDetails---->:"+JSON.stringify(response));
				if(response.code != 1){
					...
					if (dataValue != null) {
						...
						//初始化客户评论内容
						slowExpertsDetails.initCustomerComments(dataValue.ID,1,10);
						commentDID = dataValue.ID;
						//初始化客户评论上拉加载
						slowExpertsDetails.initplusscrollbottom();
					}
				}
			},
			onerror: function(e) {
	...
	initCustomerComments: function(doctorId,pageIndex,pageSize) {
		var requestCustomerComments = "doctorId=" + doctorId + "&pageIndex=" + pageIndex + "&pageSize=" + pageSize;
...
//				console.log("initCustomerComments---->:"+JSON.stringify(response));
				if(response.code != 1){
					var _divs = document.createElement('div');
					_divs.setAttribute('class','order_none');
					_divs.setAttribute('id','order_none_pl');
					_divs.innerHTML = "<i></i> 暂无评论";
					_commentdivs.appendChild(_divs);
					document.getElementById('order_none_pl').style.display = 'block';
					plus.nativeUI.alert(response.msg, '','健医宝', '确认');
					return false;
				}else{
					if(response.data != null && response.data.list != null ){
						...
						if (dataValue.length > 0) {
							document.getElementById('order_none_pl').style.display = 'none';
							...
							for (var i = 0; i < dataValue.length; i++) {
								...
								var _divs = document.createElement('div');
								_commentdivs.appendChild(_divs);
								var _commenth2s = document.createElement('h2');
								var name = dataValue[i].Name ? dataValue[i].Name : "**";
								_commenth2s.innerHTML = "	<h2>" + name + "<span>" + time + "</span></h2>";
								_divs.appendChild(_commenth2s);
								var _commentps = document.createElement('p');
								_commentps.setAttribute('class','ellipsis');
								var comment = dataValue[i].Comment ? dataValue[i].Comment : "";
								_commentps.innerText = comment;
								_divs.appendChild(_commentps);
							}
						}else{
							if(_commentdivs.innerHTML == ""){
								var _divs = document.createElement('div');
								_divs.setAttribute('class','order_none');
								_divs.setAttribute('id','order_none_pl');
								_divs.innerHTML = "<i></i> 暂无评论";
	...
	queryDocatorArranges: function(doctorId, queryDate) { //根据医生ID和日期获取该医生的号源信息
		...
		jyapp.getApi({
			...
			onerror: function(e) {
				...
				plus.nativeUI.alert('医生号源信息获取失败！', '','健医宝', '确认');
				return false;
				mui('#appointmentTime').popover('toggle');
			}
		});
	...
	editAppointment: function() {
		...
		if (!appointmentTimeValue || typeof(appointmentTimeValue) == 'undefined' || appointmentTimeValue == "") {
			plus.nativeUI.alert('请选择预约时间', '','健医宝', '确认');
			return false;
		}
	...
	rebackPreloadSlowExpertsDetails : function(){
		document.getElementById('slowExpertsDetailsSkilledID').innerHTML = '';
		document.getElementById('slowExpertsDetailsCareerID').innerHTML = '';
		document.getElementById('itemA1').setAttribute('class','mui-control-item');
		document.getElementById('itemA2').setAttribute('class','mui-control-item');
		document.getElementById('itemA3').setAttribute('class','mui-control-item');
	...
	initplusscrollbottom : function(){
		document.addEventListener("plusscrollbottom", function() {
			if(total == pageSize){
				pageIndex ++;
				//初始化客户评论内容
				slowExpertsDetails.initCustomerComments(commentDID,pageIndex,pageSize);
			}else{
				return false;
			}
		});
...
mui('.mui-content').on('tap', '.on', function() {
	...
	if (appDays == "") {
		plus.nativeUI.alert('请选择预约日期', '','健医宝', '确认');
		return false;
	}
