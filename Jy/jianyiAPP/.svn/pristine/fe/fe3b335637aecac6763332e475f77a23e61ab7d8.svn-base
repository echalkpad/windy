...
mui('.pup').on('tap', '.btn-cancel', function() {
	...
	addClass(L[L.length - 1], 'selector');
})
...
//						console.log('---' + JSON.stringify(data));
			...
			if (newDate.gender != '') {
//				console.log(newDate.gender);
				Sex.innerText = ['', '女','男'][newDate.gender];
			}
...
function getCode(data) {
	...
	if (newData.slice(0, -1) == '' || newData.slice(0, -1) == null) {
		return 0;
	}
